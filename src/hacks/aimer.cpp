...
void Aimer::aimCheck(unsigned int i) {
  ...
  try {
    ...
    view = getView(rcs);
    player_pos = local_player->viewOrigin;
    if (settings.debug)
      printVec("player_pos", player_pos);
    if (settings.debug)
      printVec("viewOrigin", local_player->viewOrigin);
    if (settings.debug)
      printVec("view", view);
    // usually closestTargetInFOV
    pair<PlayerPtr, Vector> temp = findTargetDispatcher(view, i);
    enemy = temp.first;
  ...
  if (settings.debug)
    printVec("distance vec", dist);
  if (settings.debug)
    printVec("player_pos", player_pos);
  if (settings.debug)
    printVec("target_pos", target_pos);
  ...
  MouseMovement move = mouseMovementDispatcher(
      ...
      use_smooth, i);
  if (settings.debug)
    cout << dec << "move angle x: " << move.x << endl;
  if (settings.debug)
    cout << dec << "move angle y: " << move.y << endl;
...
MouseMovement Aimer::spline_calcMouseMovement(QAngle curr_angle, Vector dist,
  ...
  //   missing_angle.y += 360;
  // if (settings.debug)
  //   printf("missing angles: x:%f y:%f\n", missing_angle.x, missing_angle.y);
  // if (i == 0) {
  //   QAngle support_angle = {(float)curr_angle.x + (float)0.25 *
  //   missing_angle.x,
  //                           (float)curr_angle.y + (float)0.75 *
  //                           missing_angle.y, 0};
  //   // spline interpolation see:
  //   // https://en.wikipedia.org/wiki/Spline_interpolation
  //   // curr_angle = x_0, y_0, support_angle = x_1, y_1, target_angle = x_2,
  //   y_2 boost::numeric::ublas::matrix<float> matrix(3, 3);
  ...
  //   // setting up vector
  //   float squared_diff1 =
  //       (support_angle.y - curr_angle.y) * (support_angle.y - curr_angle.y);
  //   float squared_diff2 =
  //       (target_angle.y - support_angle.y) * (target_angle.y -
  //       support_angle.y);
  ...
  //   spline_a[0] = y[0] * (support_angle.y - curr_angle.y) -
  //                 (support_angle.x - curr_angle.x);
  //   spline_a[1] = y[1] * (target_angle.y - support_angle.y) -
  //                 (target_angle.x - support_angle.x);
  //   spline_b[0] = -y[1] * (support_angle.y - curr_angle.y) +
  //                 (support_angle.x - curr_angle.x);
  //   spline_b[1] = -y[2] * (target_angle.y - support_angle.y) +
  //                 (target_angle.x - support_angle.x);
  ...
  //   if (j < steps / 2) {
  //     section = 0;
  //     t = (test_x - spline_start_angle.y) /
  //         (spline_supp_angle.y - spline_start_angle.y);
  //     q = (1 - t) * spline_start_angle.x + t * spline_supp_angle.x +
  //         t * (1 - t) * (spline_a[section] * (1 - t) + spline_b[section] *
  //         t);
  //   } else {
  //     section = 1;
  //     t = (test_x - spline_supp_angle.y) /
  //         (target_angle.y - spline_supp_angle.y);
  //     q = (1 - t) * spline_supp_angle.x + t * target_angle.x +
  //         t * (1 - t) * (spline_a[section] * (1 - t) + spline_b[section] *
  //         t);
  //   }
  ...
  // if ((i + 1) < steps / 2) {
  //   section = 0;
  //   t = (x - spline_start_angle.y) /
  //       (spline_supp_angle.y - spline_start_angle.y);
  //   q = (1 - t) * spline_start_angle.x + t * spline_supp_angle.x +
  //       t * (1 - t) * (spline_a[section] * (1 - t) + spline_b[section] * t);
  // } else {
  //   section = 1;
  ...
  // printf("start angle: x:%f y:%f\n", spline_start_angle.x,
  //        spline_start_angle.y);
  ...
  // int mouseAngle_x =
  //     static_cast<int>((-1 / steps) * spline_x_len * multiplier *
  //     inverse_sens);
  // int mouseAngle_y = static_cast<int>(q * multiplier * inverse_sens);
  // return {mouseAngle_x, mouseAngle_y};
}

MouseMovement Aimer::default_calcMouseMovement(QAngle curr_angle, Vector dist,
                                               bool use_smooth) {
  normalize_vector(dist);
  if (settings.debug)
    printVec("dist", dist);
...
Vector Aimer::getView(bool rcs) {
  auto local_player = csgo.getLocalPlayer();
  QAngle currAngle = local_player->networkAngle;
 ...
 */
pair<PlayerPtr, Vector> Aimer::zeusTarget(Vector view, float fov) {
  // CEntityInstance *local_player;
  ...
  //                      local_player->m_vecOrigin.y +
  //                          local_player->m_vecViewOffset.y,
  ...
  // if (settings.debug)
  //   cout << "Player count:" << players.size() << endl;
  // if (settings.debug)
  //   cout << "Local player index:" << local_player_index << endl;
  // int enemy_index = -1;
  // for (EntityType *enemy : players) {
  //   enemy_index++;
  //   if (enemy_index == local_player_index ||
  //       (!settings.aim_teammates && enemy->m_iTeamNum == team))
  //     continue;
  //   if (settings.aim_smoke_check &&
  //       csgo.lineThroughSmoke(player_pos, enemy->m_vecOrigin))
  //     continue;
  ...
  //       if (settings.debug)
  //         cout << "target too far for zeus with " << len(distVec) << endl;
  //       continue;
  //     }
...
pair<std::shared_ptr<Player>, Vector> Aimer::closestTargetInFov(Vector view,
                                                                float fov) {
  PlayerPtr local_player;
  int local_player_index = csgo.getLocalPlayerIndex();
  Vector bone_pos;
  ...
  Vector player_pos = local_player->viewOrigin;
  vector<PlayerPtr> players = csgo.getPlayers();
  ...
  PlayerPtr closestPlayer = nullptr;
  if (settings.debug)
    cout << "Fov: " << fov << endl;
  float closestAngle = fov;
  Vector closestBone = {0, 0, 0};
  Team team = local_player->team;
  if (settings.debug)
    cout << "Player count:" << players.size() << endl;
  if (settings.debug)
    cout << "Local player index:" << local_player_index << endl;
  ...
  for (PlayerPtr enemy : players) {
    enemy_index++;
    if (settings.debug)
      cout << "Checking enemy:" << enemy_index << endl;
    if (enemy_index == local_player_index ||
        (!settings.aim_teammates && enemy->team == team))
      continue;
    // if (settings.aim_smoke_check &&
    //     csgo.lineThroughSmoke(player_pos, enemy->m_vecOrigin)) {
    //   if (settings.debug)
    //     cout << "Ignoring player " << enemy_index << " due to smoke." <<
    //     endl;
    //   continue;
    // }
...
Vector Aimer::predictPositionOffset(PlayerPtr) {
  // Vector vel = {player->m_vecVelocity.y, player->m_vecVelocity.z,
