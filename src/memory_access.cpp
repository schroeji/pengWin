...
void MemoryAccess::updateAddrs() {
  ...
  read((void*) local_player_addr_location, &local_player_addr, sizeof(local_player_addr));
...
unsigned int MemoryAccess::getCrosshairTarget() {
  unsigned int ret;
  read((void*) (local_player_addr + crosshair_id_offset), &ret, sizeof(ret));
  return ret;
}

Team MemoryAccess::getTeam() {
  unsigned int team;
  read((void*) (local_player_addr + team_number_offset), &team, sizeof(team));
  return (Team) team;
}
