...
// no aimbot adjustment if current view vectorintersects a sphere with this
// radius around one of the target bones
...
Vector getDist(Vector const &, Vector const &);
float scalar_prod(Vector const &, Vector const &);
float scalar_prod(Vector2D const &, Vector2D const &);
Vector cross_prod(Vector const &, Vector const &);
float sgn(float);
void solve(boost::numeric::ublas::matrix<float> *,
           boost::numeric::ublas::vector<float> *);

inline float degree_to_radian(float x) { return x * 0.01745329; }

inline float radian_to_degree(float x) { return x / 0.01745329; }
...
inline float len(Vector2D vec) { return sqrt(vec.x * vec.x + vec.y * vec.y); }
